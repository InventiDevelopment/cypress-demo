...
const webhookInitialArgs = async ({
    ...
    if (!commitUrl) {
        ...
        if (!ciEnvVars.CI_USERNAME) {
            triggerText = `This run was triggered by <${commitUrl}|commit> in branch ${process.env.BITBUCKET_BRANCH}`;
        } else {
    ...
    projectName = `Test suite: ${process.env.BRAND}/${process.env.COUNTRY}/${process.env.ENVI}/${process.env.TEST_TYPE}`;
    return {
...
const attachmentReports = async ({
    ...
    console.log('Branch text -> ',ciEnvVars.CI_BRANCH)
...
const buildHTMLReportURL = async ({
    ...
    return artefactUrl + reportDir + "/" + reportHTMLFilename +'?versionId='+ objectVersionId;
};
